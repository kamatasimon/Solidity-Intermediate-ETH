...
import ZamaToken from "../abi/ZamaToken.json";
...
export default function Token() {
  ...
  useEffect(() => {
    ...
    web3Modal.connect().then((provider) => {
      ...
      zamaContract.totalSupply().then(function (totalSupply) {
  ...
  const mintTokens = async () => {
    ...
    try {
      ...
      await tx.wait();
      setMintValue('');
      console.log(`Minted ${value} Zama tokens to ${to}`);
      alert(`Minted ${value} ZamaTokens to ${to}`);
    } catch (error) {
  ...
  const burnTokens = async () => {
    ...
    try {
      ...
      await tx.wait();
      setBurnValue('');
      console.log(`Burned ${value} Zama tokens`);
      alert(`Burned ${value} Zama tokens`);
    } catch (error) {
      console.error(`Error burning Zama tokens: ${error}`);
      alert(`Error burning Zama tokens: ${error}`);
    }
  };
